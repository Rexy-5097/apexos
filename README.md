...
[![Live Demo](https://img.shields.io/badge/{{YOUR_DOMAIN}}-00E5FF?style=for-the-badge&logo=vercel&logoColor=white)](https://{{YOUR_DOMAIN}})
...
[Live Demo](https://{{YOUR_DOMAIN}}) · [Report Bug](https://github.com/{{YOUR_GITHUB_USERNAME}}/apexos/issues) · [Request Feature](https://github.com/{{YOUR_GITHUB_USERNAME}}/apexos/issues)
...
ApexOS is a production-grade OSINT platform that provides situational awareness across multiple intelligence domains. Built with Next.js 16 and MapLibre GL, every data point is rendered via WebGL for 60fps performance even with thousands of concurrent entities on-screen.
...
`node:22-alpine` standalone build (~220 MB, non-root). See **[DOCKER.md](DOCKER.md)** for the full Docker and API-key guide.

**Prebuilt image** — pull it directly:

```bash
docker pull {{YOUR_REGISTRY}}/apexos:latest
docker run -d -p 3000:3000 --env-file .env {{YOUR_REGISTRY}}/apexos:latest
```

**Custom port** — the container always listens on `3000`; set `PORT` in
`.env` to change the published host port (e.g. `PORT=3005`) without
editing the compose file.

### Environment Variables

ApexOS works **partially without any API keys** — all core feeds use public,
...
# SCANNER_KEY must match the backend's key — generate with: openssl rand -hex 32
SCANNER_URL=
SCANNER_KEY=
